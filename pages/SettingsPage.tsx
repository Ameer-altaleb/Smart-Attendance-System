...
const SettingsPage: React.FC = () => {
  ...
  // Sync local state when global settings change (Real-time sync)
  useEffect(() => {
  ...
  const runDiagnosticAnalysis = async () => {
    ...
    const steps = [
      ...
      { msg: `حالة القناة: ${isRealtimeConnected ? 'متصلة (Active)' : 'منقطعة (Disconnected)'}`, type: isRealtimeConnected ? 'success' : 'error' },
      { msg: 'تحليل متغيرات البيئة (API_KEY, URL)...', type: 'info' },
      { msg: 'فحص استجابة الجداول (Ping Tables)...', type: 'info' },
      { msg: `قاعدة البيانات: ${dbStatus.settings === 'online' ? 'جاهزة' : 'فحص الاتصال...'}`, type: 'info' },
    ...
    try {
      await refreshData();
      addLog('تم تحليل الجداول: centers, employees, admins, attendance, settings', 'success');
  ...
  const handleSaveSystem = async (e: React.FormEvent) => {
    ...
    addLog(`إرسال تحديث المتغيرات للهوية الجديدة...`, 'command');

    try {
      await updateSettings({ ...settings, systemName: sysName, logoUrl, language: lang, timeFormat });
      addLog('تم تحديث إعدادات النظام وتعميمها عبر السحابة.', 'success');
      triggerSuccess('تم حفظ إعدادات النظام');
    } catch (err) {
      addLog('فشل في حفظ الإعدادات، يرجى المحاولة لاحقاً', 'error');
    } finally {
